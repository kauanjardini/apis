...
try {
var kauan = db.get("Kauan");
} catch (e) {
	console.log('')
}

var criadorList = ['@kauanjardini'];
...
loghandler = {
    notparam: {
        ...
        criador: `${criador}`,
        code: 406,
        message: 'cade o parametro apikey',
        getApikey: 'apikey invalida! afim de comprar a apikey entre em contato comigo em: +554491150998'
    },
    notkey: {
        status: false,
        criador: `${criador}`,
        code: 406,
        message: 'cade o parametro key'
    },
    noturl: {
        status: false,
        criador: `${criador}`,
    ...
    nottext2: {
        ...
        criador: `${criador}`,
        code: 406,
        message: 'cade o parametro text2'
    },
    notnabi: {
        status: false,
        criador: `${criador}`,
        code: 406,
        message: 'cade o parametro nabi'
    },
    nottext3: {
        status: false,
        criador: `${criador}`,
        code: 406,
        message: 'cade o parametro text3'
    },
    nottheme: {
        status: false,
        criador: `${criador}`,
        code: 406,
        message: 'cade o parametro theme'
    },
    notusername: {
        status: false,
        criador: `${criador}`,
        code: 406,
        message: 'cade o parametro username'
    },
    notvalue: {
        status: false,
        criador: `${criador}`,
        code: 406,
        message: 'cade o parametro value'
    },
    notheme: {
    	status: false,
        criador: `${criador}`,
        code: 406,
        message: 'informe um tema de texmaker/list para documento'
     },
    invalidKey: {
        status: false,
        criador: `${criador}`,
        code: 406,
        message: 'apikey invalida, precisa de apikey? me chame em wa.me/554491150998'
    },
    invalidlink: {
        status: false,
        criadorr: `${criador}`,
        message: 'erro, informe um link valido.'
    },
    invalidkata: {
        status: false,
        criador: `${criador}`,
        message: 'erro, informe um kata valido para a api.'
    },
    notAddApiKey: {
        status: false,
        criador: `${criador}`,
        code: 406,
        message: 'cade os parametros status, apikeyInput, email, nomorhp, name, age, country, exp'
    },
    error: {
        status: false,
        criador: `${criador}`,
...
var len = 15
        ...
        var randomTextNumber = random+randomlagi+'---------kauanjardini'+'K';
        
 
 async function cekApiKey(api) {
 	ap = await kauan.findOne({apikey:api})
 return ap;
 }
router.get('/find', async (req, res, next) => {
    var apikey = req.query.apikey
    if (!apikey) return res.json(loghandler.notparam)
    if (apikey != 'kauan') return res.json(loghandler.invalidKey)
...
router.get('/tiktod', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'kauan') return res.json(loghandler.invalidKey)
...
router.get('/infonpm', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'kauan') return res.json(loghandler.invalidKey)
    if (!query) return res.json({ status : false, criador : `${criador}`, message : "masukan parameter query"})
...
router.get('/base', async (req, res, next) => {
	var type = req.query.type,
		...
		if (!apikeyInput) return res.json(loghandler.notparam)
		if (apikeyInput != 'kauan') return res.json(loghandler.invalidKey)
		if (!type) return res.json({status: false, criador, code: 404, message: 'masukan parameter type, type yang tersedia : base64 , base32'})
		if (type == 'base64' && encode){
				...
				res.json({
					status:false,
					criador: `${criador}`,
					message: "cade o parametro encode/decode"
				})
			} else {
				res.json(loghandler.error)
			}
...
router.get('/hadits', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'kauan') return res.json(loghandler.invalidKey)
    if (!kitab) return res.json({ status : false, criador : `${criador}`, message : "masukan parameter kitab"})
    if (!nomor) return res.json({ status : false, criador : `${criador}`, message : "masukan parameter nomor"})
...
router.get('/quran', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'kauan') return res.json(loghandler.invalidKey)
    if (!surah) return res.json({ status : false, criador : `${criador}`, message : "masukan parameter surah"})
    if (!ayat) return res.json({ status : false, criador : `${criador}`, message : "masukan parameter ayat"})
...
router.get('/fbdown', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'kauan') return res.json(loghandler.invalidKey)
    if (!url) return res.json({ status : false, criador : `${criador}`, message : "masukan parameter url"})
...
router.get('/wikipedia', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'kauan') return res.json(loghandler.invalidKey)
        if(!search) return res.json({ status : false, criador : `${criador}`, message : "masukan parameter search"})
...
router.get('/jadwalshalat', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'kauan') return res.json(loghandler.invalidKey)
        if(!kota) return res.json({ status : false, criador : `${criador}`, message : "masukan parameter kota"})
...
router.get('/fakedata', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'kauan') return res.json(loghandler.invalidKey)
        if(!country) return res.json({ status : false, criador : `${criador}`, message : "masukan parameter country"})
...
router.get('/hilih', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'kauan') return res.json(loghandler.invalidKey)
        if(!kata) return res.json({ status : false, criador : `${criador}`, message : "masukan parameter kata"})
...
router.get('/liriklagu', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'kauan') return res.json(loghandler.invalidKey)
        if(!lagu) return res.json({ status : false, criador : `${criador}`, message : "masukan parameter kata"})
...
router.get('/cuaca', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'kauan') return res.json(loghandler.invalidKey)
	if(!kabupaten) return res.json({ status : false, criador : `${criador}`, message : "masukan parameter kabupaten"})
